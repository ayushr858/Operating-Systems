...
// Bits 8-9 of a leaf PTE are reserved for software (RSW) use by the
// RISC-V privileged spec. We use one of them to mark copy-on-write
// pages: pages installed read-only in a child (and its parent) after
// fork(), to be duplicated lazily on the first write.
#define PTE_COW (1L << 8)
